import { AI_NAME, OWNER_NAME } from "@/configuration/identity";

export const CHAT_HEADER: string = `Chat with Amani`;
export const MESSAGE_PLACEHOLDER: string = `Hello, I'm Amani, Your personal Human Rights Assistant! I'm here to help with your questions about Human Rights!`;
export const FOOTER_MESSAGE: string = `Customized AI by Caden Alford @ UNC`;
export const CLEAR_BUTTON_TEXT: string = `Clear`;
export const PAGE_TITLE: string = `Amani`;
export const PAGE_DESCRIPTION: string = `Chat with ${AI_NAME}, ${OWNER_NAME}'s AI assistant.`;

export const EMPTY_CITATION_MESSAGE: string = "Unspecified source";
